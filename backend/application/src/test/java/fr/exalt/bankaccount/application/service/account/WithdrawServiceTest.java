...
public class WithdrawServiceTest {
    ...
    void withdraw_should_decrease_balance_and_persist() {
        ...
        WithdrawService service = new WithdrawService(accountRepository, operationRepository);
        ...
        assertThat(op.type()).isEqualTo(Operation.Type.WITHDRAWAL);
    }
    ...
    @Test
    void withdraw_exceeding_overdraft_should_propagate_overdraft_exception() {
        AccountRepository accountRepository = new InMemoryAccountRepository();
        OperationRepository operationRepository = new InMemoryOperationRepository();

        WithdrawService service = new WithdrawService(accountRepository, operationRepository);
        Account account = Account.openCurrent(Money.of("-100"), clock);
        accountRepository.save(account);

        WithdrawCommand cmd = new WithdrawCommand(account.getId(), Money.of("102"));
